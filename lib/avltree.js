...
_AVLTree.prototype.insert = function (key, value) {
  ...
  for (i = insertPath.length - 2; i >= 0; i -= 1) {
    insertPath[i].height = Math.max(insertPath[i].height, insertPath[i+1].height + 1);

    if (insertPath[i].balanceFactor() > 1) {
      rotated = insertPath[i].rightRotation();
      if (i === 0) { newRoot = rotated; }
    }

    if (insertPath[i].balanceFactor() < -1) {
      rotated = insertPath[i].leftRotation();
      if (i === 0) { newRoot = rotated; }
    }
  }
