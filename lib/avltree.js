 ...
 */
_AVLTree.prototype.checkIsBST = function () {
...
AVLTree.prototype.checkIsBST = function () { this.tree.checkIsBST(); };
...
_AVLTree.prototype.insert = function (key, value) {
  ...
  for (i = insertPath.length - 1; i >= 0; i -= 1) {
    insertPath[i].height = 1 + Math.max(insertPath[i].left ? insertPath[i].left.height : 0, insertPath[i].right ? insertPath[i].right.height : 0);

    if (insertPath[i].balanceFactor() > 1) {
      rotated = insertPath[i].rightTooSmall();
      if (i === 0) { newRoot = rotated; }
    }

    if (insertPath[i].balanceFactor() < -1) {
      rotated = insertPath[i].leftTooSmall();
      if (i === 0) { newRoot = rotated; }
    }
  }
 ...
 */
['getNumberOfKeys', 'search', 'prettyPrint'].forEach(function (fn) {
